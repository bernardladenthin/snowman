...
/**
 * Class for a single cameraviewer.
 */
class Cameraviewer
{
    private $camera;
    ...
    public final function loadSpecificImage($filename, $fileerrormessage = true)
    {
        ...
        $givenwidth = $width = $this->getCamera()->getCCamera()->getWidth();
        $givenheight = $height = $this->getCamera()->getCCamera()->getHeight();
     ...
     * @param bool $fileerrormessage
     * @return String filename of loaded image
     */
    ...
    private final function createFileErrorMessage($filename)
    {
        ...
        $this->createMessage(
            ...
            0,
            $yOffset,
            $this->getCamera()->getCCamera()->getWidth(),
            $yOffset + 10
        );
    }
    ...
    public final function createWatermark($POSIXMillis = false, $yOffset = 0)
    {
        ...
        $msg = $this->getCamera()->getCCamera()->getTopTextBranding();
        $msg .= $date;
    ...
    {
        $msg = $this->getCamera()->getCCamera()->getBottomTextBranding();
        $this->createMessage(
            $msg,
            0,
            $this->getCamera()->getCCamera()->getHeight() - 10,
            $this->getCamera()->getCCamera()->getWidth(),
            $this->getCamera()->getCCamera()->getHeight()
        );
    }
